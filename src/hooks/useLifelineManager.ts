...
export const useLifelineManager = ({
	...
	const [activeLifelineModal, setActiveLifelineModal] =
		useState<LifelineInfo | null>(null);
	...
	const onSimplify = () =>
		triggerLifeline("simplify", () => setIsSimplifyActive(true));

	const onSkip = () =>
		triggerLifeline("skip", () => nextQuestion(state.difficulty));
