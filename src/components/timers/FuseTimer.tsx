...
export const FuseTimer = ({ timeLeft, totalTime, className }: TimerProps) => {
    ...
    return (
        <div className={`relative w-full max-w-[200px] flex flex-col items-center gap-2 ${className}`}>
                ...
                {timeLeft > 0 && (
                    <motion.div
                        ...
                        animate={{
                            left: `${(1 - progress) * 100}%`,
                            scale: [1, 1.5, 1],
                            boxShadow: ["0 0 10px #fbbf24", "0 0 25px #f59e0b", "0 0 10px #fbbf24"]
                        }}
                        transition={{
            ...
            <span className={`text-xl font-mono font-black ${timeLeft < 10 ? "text-error" : "text-white"}`}>
